...
<template>
  <div
    ...
    <div class="flex items-center gap-2">
      <img class="w-10 h-10 rounded-full border" :src="avatar" :alt="name" width="40" height="40" loading="lazy"
        decoding="async">
      <div class="flex flex-col justify-center">
        <span>{{ name }}</span>
        <span v-if="badgeText"
          class="mt-0.5 w-fit text-[11px] leading-4 px-1.5 rounded-full border border-amber-500/20 bg-amber-500/8 text-amber-700 dark:text-amber-300">
          {{ badgeText }}
        </span>
      </div>
    </div>
